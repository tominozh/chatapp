...
/*
 * Client class, when connection is accepted ClientHandler gets instantiated, as
 * long as conncetion is alive it echoes message send to all other clients. if
 * client enters "/quit" object gets destroyed and its place in clientThreads[]
 * is set to null. If client close terminal, NullPointerException is thrown and
 * handled
 */
	...
	public void run() {
		...
		try {
			...
			try {
				...
				System.out.println("cant get semaphore - letting know about new client");			     
			}
			finally {
				semaphore.release();
			}
			...
			while (true) {
					...
					if (!line.isEmpty()) {
						...
						if (line.startsWith("/quit")) {
							for (int i = 0; i < maxClientsCount; i++) {
								if (clientThreads[i] != null && clientThreads[i] != this) {
									clientThreads[i].outputStream
											.println("The user " + this.clientName + " left the chat room.");
								}
